    ...
    <>
      ...
      <motion.nav
        ...
        className={`fixed top-0 left-0 right-0 z-[100] px-6 py-4 md:px-12 flex justify-between items-center transition-all duration-300 ${
          isScrolled
            ? 'backdrop-blur-md bg-bgPrimary/75 dark:bg-bgSecondary/75 border-b border-glass-border shadow-xl'
            : 'bg-transparent'
        }`}
      >
        {/* Brand Logo */}
        ...
        {/* Desktop Links */}
        <ul className="hidden md:flex gap-1 items-center bg-black/5 dark:bg-white/5 p-1 rounded-full border border-glass-border">
          {NAV_LINKS.map((link) => (
            <li key={link.name}>
              <a
                href={link.href}
                ...
                onMouseEnter={() => playAudio('hover')}
                className="px-4 py-1.5 rounded-full text-xs font-semibold text-textSecondary hover:text-textPrimary hover:bg-black/5 dark:hover:bg-white/10 transition-all duration-200 block"
              >
                {link.name}
              </a>
            </li>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-x-0 top-[72px] z-[90] p-6 bg-bgPrimary/95 dark:bg-bgSecondary/95 border-b border-glass-border backdrop-blur-xl flex flex-col gap-3 md:hidden shadow-2xl"
          >
            {NAV_LINKS.map((link) => (
              <a
                key={link.name}
                ...
                onMouseEnter={() => playAudio('hover')}
                className="px-4 py-3 rounded-xl text-sm font-semibold text-textSecondary hover:text-textPrimary hover:bg-black/5 dark:hover:bg-white/5 transition-all duration-200 block"
              >
                {link.name}
              </a>
            ))}
